...
export class CustomField extends React.Component<IProps> {
  ...
  componentDidMount() {
    const {id, isEditScreen, dispatch, getCustomField} = this.props;
    ...
    if (isEditScreen) {
      getCustomField({
        ...
        onResult: res => {
          const field = {
            ...res,
            [FIELDS.DEFAULT_VALUE]: res.defaultAnswer || res.default_answer
          };
  ...
  isLoading = () => {
    ...
    return !formValues || !hasObjectLength(formValues) || getCustomFieldLoading;
  };
